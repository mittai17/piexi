...
export const SearchBar: React.FC<SearchBarProps> = ({ query, setQuery, onSearch, isLoading, searchFocus, setSearchFocus, hasSearched }) => {
  ...
  return (
    <div className={`relative flex items-center w-full rounded-full transition-all duration-300 border border-gray-700 bg-gray-900/60 backdrop-blur-md shadow-lg ${isFocused ? 'ring-2 ring-purple-600' : ''}`}>
        {/* Focus Dropdown Button */}
        <div ref={dropdownRef} className="relative">
          <button
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            className="flex items-center gap-2 pl-4 pr-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700/50 rounded-l-full"
          >
          ...
          {/* Dropdown Menu */}
          {isDropdownOpen && (
            <div className="absolute top-full mt-2 w-48 bg-gray-800 border border-gray-700 rounded-lg shadow-xl animate-fade-in-up-fast z-20">
        ...
        <input
          ...
          placeholder={hasSearched ? 'Ask a follow-up or enter a URL...' : 'Ask anything or enter a URL...'}
          disabled={isLoading}
          className="w-full p-3 pl-4 pr-14 text-sm bg-transparent outline-none transition-all duration-300 text-white placeholder-gray-500"
        />

        <button
          onClick={onSearch}
          disabled={isSearchDisabled}
          className="absolute right-1.5 top-1/2 -translate-y-1/2 p-2 rounded-full text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-purple-500 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors duration-300"
          aria-label="Search"
        >
